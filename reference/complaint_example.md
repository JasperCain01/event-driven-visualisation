...
stage-process features (\`patient_col = NULL\`,
\[plot_stage_ladder()\]). Sprinkled point events (\`contact\`,
\`escalation\`, \`evidence_received\`) exercise the instantaneous-event
path. Complaint \`"CMP-03"\` stalls about three weeks in "Under review"
(a per-stage breach); \`"CMP-04"\` is still open and never reaches the
formal letter, exercising the ongoing-spell indication.

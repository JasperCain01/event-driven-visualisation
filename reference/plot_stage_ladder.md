...
\[plot_case_timeline()\] uses; only the rendering differs.
...
- state_events:
  ...
  Character vector of \`act_type\` value(s) marking a stage entry.
  Required — no default; see \[plot_case_timeline()\] for the
...
- time_col, act_type_col, activity_col, case_col:
  ...
  Column-name mappings, as in \[plot_case_timeline()\]. \`case_col\` may
  be \`NULL\` for a single unnamed series (\`case_id\` must then also be
  \`NULL\`).
...
- stage_targets:
  ...
  (\[summarise_breach_rate()\] instead sums a case's visits into one
  total dwell). An open (end-inferred) stage draws only its \*proven\*
  excess — dwell observed up to the last recorded event, never the
  imputed end. \`NULL\` = no targets shown.

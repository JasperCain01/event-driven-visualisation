...
#
# Covers plot_cohort_timeline(): panel counts for NULL vs subset case_ids, the
# align_start rebase (every case's first box at elapsed hour 0), cross-facet
# colour consistency (same state -> same fill in every panel), the max_cases
...
# ── Shared fixture — 3 cases sharing a common vocabulary of states ─────────────
...
cohort <- function(...) plot_cohort_timeline(cohort_log(), state_events = STATE_EVENTS, ...)
...
test_that("align_start puts every case's first box at elapsed hour 0", {
  res <- cohort(align_start = TRUE, return_data = TRUE)
  firsts <- res$boxes |>
...
test_that("align_start does not error when a case has zero point events", {
  ...
  expect_no_error(
    p <- plot_cohort_timeline(log_no_events, state_events = STATE_EVENTS, align_start = TRUE)
  )
  expect_s3_class(p, "ggplot")
...
test_that("the same state gets the same fill hex in every panel", {
  p  <- cohort(return_data = TRUE)$plot
  bd <- ggplot2::ggplot_build(p)
  # Layer 1 is the geom_rect box layer; join its fill back to the box state.
  rect_data <- bd$data[[1]]
  boxes     <- cohort(return_data = TRUE)$boxes
  boxes     <- dplyr::filter(boxes, !terminal)

  # Same number of rects as non-terminal boxes, and each state resolves to
  # exactly one fill colour across all panels.
  expect_equal(nrow(rect_data), nrow(boxes))
  by_state <- tibble::tibble(state = boxes$state, fill = rect_data$fill) |>
    dplyr::group_by(state) |>
    dplyr::summarise(n_hex = dplyr::n_distinct(fill), .groups = "drop")
  expect_true(all(by_state$n_hex == 1L))
})
...
test_that("absolute cohort renders cleanly (universal gate)", {
  p <- cohort(show_duration = TRUE)
  expect_s3_class(p, "ggplot")
  expect_no_warning(ggplot2::ggplot_build(p))
})

test_that("start-aligned cohort renders cleanly (universal gate)", {
  p <- cohort(align_start = TRUE, show_duration = TRUE)
  expect_s3_class(p, "ggplot")
  expect_no_warning(ggplot2::ggplot_build(p))
})
...
test_that("terminal_activities is forwarded per case", {
  ...
  res <- plot_cohort_timeline(log, state_events = STATE_EVENTS,
...
test_that("absolute-mode gaps are sized per panel, not against the cohort calendar span", {
  # Two short cases five months apart. Sizing the gap (and the minimum
  ...
  p    <- plot_cohort_timeline(far, state_events = STATE_EVENTS)
...
test_that("the 'Other' bucket is coloured from the palette, not na.value grey", {
  ...
  p <- suppressMessages(plot_cohort_timeline(log, state_events = STATE_EVENTS, event_type_top_n = 2))
...
# ── Per-panel ongoing-case markers ──────────────────────────────────────────────
...
test_that("open cases get a per-panel '(ongoing)' marker; closed cases do not", {
  ...
  # exactly those two panels must carry the open-case annotation.
  log <- dplyr::bind_rows(
    cohort_log(),
    tibble::tibble(
      case_id = "C1",
  ...
  p <- plot_cohort_timeline(log, state_events = STATE_EVENTS, terminal_activities = "Discharged")
  built <- ggplot2::ggplot_build(p)
...
test_that("ongoing markers render in start-aligned mode too (universal gate)", {
  p <- cohort(terminal_activities = "Discharged", align_start = TRUE)
  built <- ggplot2::ggplot_build(p)
...
test_that("tail_strategy is forwarded per case", {
  ...
  res_fixed <- cohort(case_ids = "C1", tail_strategy = "fixed", return_data = TRUE)
  expect_equal(
    as.numeric(max(res_fixed$boxes$xmax) - hrs(3), units = "mins"), 30)
  res_med <- cohort(case_ids = "C1", return_data = TRUE)
  expect_equal(
    as.numeric(max(res_med$boxes$xmax) - hrs(3), units = "hours"), 3)
})

# ── case_col = NULL is not supported for a cohort ───────────────────────────────

test_that("case_col = NULL aborts: a cohort needs a case column", {
  expect_error(
    plot_cohort_timeline(cohort_log(), state_events = STATE_EVENTS, case_col = NULL),
    regexp = "case_col"
  )
})

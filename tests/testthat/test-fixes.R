...
test_that("show_labels = TRUE renders without dropping label rows", {
  ...
  # The repel layer must carry every event (4 non-state rows)
...
test_that("zero-width box stores true duration 0, and the message says so", {
  expect_message(
    r <- plot_case_timeline(same_ts_log(), case_id = "X", state_events = STATE_EVENTS,
                            return_data = TRUE),
    regexp = "true value"
  )
...
# ── Defect 5b: excluding all state categories crashed cryptically ─────────────

test_that("exclude_categories removing all state events aborts cleanly", {
  expect_error(
    suppressMessages(
      plot_case_timeline(small_log(), case_id = "SP-001", state_events = STATE_EVENTS,
...
test_that("vector case_id aborts with a clear message", {
  expect_error(
    plot_case_timeline(small_log(), case_id = c("SP-001", "SP-002"), state_events = STATE_EVENTS),
    regexp = "single"
  )
})
...
test_that("NA case_id aborts with a clear message", {
  expect_error(
    plot_case_timeline(small_log(), case_id = NA_character_, state_events = STATE_EVENTS),
    regexp = "single"
  )
})
...
# ── case_col = NULL support ─────────────────────────────────────────────────

test_that("case_col = NULL works for a single unnamed series", {
  log <- small_log() |> dplyr::select(-case_id)
